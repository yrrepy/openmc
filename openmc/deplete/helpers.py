...
# ---------------------------------------------------------------------------
# The GENDF helper classes live in ``openmc.deplete.gendf.helpers``, which
# imports from this module, so they are resolved lazily through ``__getattr__``
# (a module-level import would cycle).
# ---------------------------------------------------------------------------
_GENDF_HELPERS = (
    'DirectWithFluxHelper', 'GENDFFluxCollapseHelper', 'IsomericBranchingHelper')


def __getattr__(name):
    """Resolve the GENDF helper classes without importing them eagerly."""
    if name in _GENDF_HELPERS:
        from .gendf import helpers as gendf_helpers
        return getattr(gendf_helpers, name)
